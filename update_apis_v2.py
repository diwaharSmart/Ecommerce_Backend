...
if request.user.is_authenticated:
    try:
        ...
        # Calculate new wallet balance (Total Binary + Total Level - Withdrawn)
        ...
        withdrawn_total = Transaction.objects.filter(
            user=request.user, type='withdrawal', direction='debit'
        ).aggregate(Sum('amount'))['amount__sum'] or 0.00
        
        new_wallet_balance = (binary_income_total + level_income_total) - withdrawn_total
...
def get_descendants(profile):
    ...
    while current_level:
        children = list(Profile.objects.filter(parent__in=current_level))
        descendants.extend(children)
        current_level = children
    return [d.user for d in descendants]
...
if request.user.is_authenticated:
    try:
        ...
        # --- 5. PV Overview (Left / Right for Today/Week) ---
        ...
        # Calculate dynamic PV for today and this week
        ...
        if left_child:
            left_users = [left_child.user] + get_descendants(left_child)
            # Today
            today_left_pv = Order.objects.filter(
                user__in=left_users, status__in=['paid', 'completed'], created_at__gte=start_of_today
            ).aggregate(Sum('total_pv'))['total_pv__sum'] or 0
            # Week
            week_left_pv = Order.objects.filter(
                user__in=left_users, status__in=['paid', 'completed'], created_at__gte=start_of_week
            ).aggregate(Sum('total_pv'))['total_pv__sum'] or 0
            
        if right_child:
            right_users = [right_child.user] + get_descendants(right_child)
            # Today
            today_right_pv = Order.objects.filter(
                user__in=right_users, status__in=['paid', 'completed'], created_at__gte=start_of_today
            ).aggregate(Sum('total_pv'))['total_pv__sum'] or 0
            # Week
            week_right_pv = Order.objects.filter(
                user__in=right_users, status__in=['paid', 'completed'], created_at__gte=start_of_week
            ).aggregate(Sum('total_pv'))['total_pv__sum'] or 0
        ...
        data = {
            ...
            "binary_pv_overview": {
                "today": {"left": today_left_pv, "right": today_right_pv},
                "this_week": {"left": week_left_pv, "right": week_right_pv},
                "all_time": {"left": all_time_left, "right": all_time_right},
                "current_carry_forward": {"left": current_left, "right": current_right}
            },
            "total_daily_income": float(today_total),
            "total_weekly_income": float(weekly_total),
            "team_stats": {
            ...
            "level_income_overview": [],  # Or fill with level details if needed
