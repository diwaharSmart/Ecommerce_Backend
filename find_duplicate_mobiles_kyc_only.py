...
def find_duplicates():
    ...
    report_lines.append(f"{'KYC VERIFIED ACCOUNTS REPORT (GROUPED BY MOBILE)' :^80}")
    report_lines.append("="*80)
    ...
    for phone, group in all_groups.items():
        ...
        # --- FILTER: ONLY INCLUDE IF KYC IS PRESENT ---
        if shared_bank_acc == "N/A":
            continue
            
        kyc_groups_count += 1
                
        # --- 2. Print User Data ---
        report_lines.append(f"\n[ MOBILE NUMBER ]: {phone} (Accounts: {len(group)}, Sharing KYC: YES)")
    ...
    print(f"Generated report for {kyc_groups_count} unique mobile numbers WITH KYC.")
    
    filepath = "kyc_yes_accounts_report.txt"
